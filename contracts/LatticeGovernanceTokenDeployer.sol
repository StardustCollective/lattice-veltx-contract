...
import "./LatticeGovernanceToken.sol";

/**
 * @title LatticeGovernanceTokenDeployer
 * @dev Deploys LatticeGovernanceToken using CREATE2 for deterministic addresses
 */
contract LatticeGovernanceTokenDeployer {
    ...
    function computeAddress(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encodePacked(
                        type(LatticeGovernanceToken).creationCode,
                        abi.encode(ltxToken)
                    )
                )
            )
    ...
    function deployDeterministic(
        address ltxToken,
        bytes32 salt
    ) public returns (LatticeGovernanceToken deployed) {
        deployed = new LatticeGovernanceToken{salt: salt}(
            IERC20(ltxToken)
        );
