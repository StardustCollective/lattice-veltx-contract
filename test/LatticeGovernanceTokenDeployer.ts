...
describe('LatticeGovernanceTokenDeployer', function () {
  const deployDeployer = async () => {
    ...
    const veltxTokenV1 = await LatticeGovernanceTokenV1Factory.deploy(
      await ltxToken.getAddress(),
    );

    const LatticeGovernanceTokenV2Factory = await ethers.getContractFactory(
      'LatticeGovernanceTokenV2',
    );
    const veltxTokenV2 = await LatticeGovernanceTokenV2Factory.deploy(
      await ltxToken.getAddress(),
  ...
  describe('Compute Address', async () => {
    it('Computes deterministic address', async () => {
      const { deployer, ltxToken, veltxTokenV2 } =
    ...
    it('Returns same address for same inputs', async () => {
      ...
      const v2Address = await veltxTokenV2.getAddress();
  ...
  describe('Deploy', async () => {
    it('Deploys contract to predicted address', async () => {
      const { deployer, ltxToken, veltxTokenV2, userAccountA } =
        await networkHelpers.loadFixture(deployDeployer);
      ...
      const trx = await deployer
        ...
        .deployDeterministic(ltxAddress, v2Address, salt);
    ...
    it('Different users can deploy with same salt', async () => {
      const { deployer, ltxToken, veltxTokenV2, userAccountA, userAccountB } =
        await networkHelpers.loadFixture(deployDeployer);
      ...
      await deployer.computeAddress(ltxAddress, v2Address, salt);
      ...
      await expect(
        deployer
          .connect(userAccountB)
          .deployDeterministic(ltxAddress, v2Address, salt2),
  ...
  describe('Is Deployed', async () => {
    it('Returns false before deployment', async () => {
      ...
      const ltxAddress = await ltxToken.getAddress();
      const v2Address = await veltxTokenV2.getAddress();
      const isDeployed = await deployer.isDeployed(ltxAddress, v2Address, salt);
    ...
    it('Returns true after deployment', async () => {
      ...
      // Before deployment
      expect(await deployer.isDeployed(ltxAddress, v2Address, salt)).to.equal(
        false,
      );
    ...
    it('Returns false for different salt', async () => {
      ...
      await deployer
        .connect(userAccountA)
        .deployDeterministic(ltxAddress, v2Address, salt1);

      // Check both salts
      expect(await deployer.isDeployed(ltxAddress, v2Address, salt1)).to.equal(
        true,
      );
      expect(await deployer.isDeployed(ltxAddress, v2Address, salt2)).to.equal(
        false,
      );
    });
  });
  ...
  describe('Deterministic Address Verification', async () => {
    it('Multiple deployers produce different addresses for same salt', async () => {
      ...
      const address1 = await deployer1.computeAddress(
        ltxAddress,
        v2Address,
        salt,
      );
      const address2 = await deployer2.computeAddress(
        ltxAddress,
        v2Address,
        salt,
      );
    ...
    it('Same deployer produces same address across calls', async () => {
      ...
      const addresses = await Promise.all([
        deployer.computeAddress(ltxAddress, v2Address, salt),
        deployer.computeAddress(ltxAddress, v2Address, salt),
        deployer.computeAddress(ltxAddress, v2Address, salt),
      ]);
